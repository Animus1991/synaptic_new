...
export function LeitnerOcclusionFace({ occlusion, flipped }: Props) {
  ...
  return (
    ...
    >
      ...
      <p className="relative z-10 px-3 pt-2 type-caption text-text-muted truncate">{sourceFileName}</p>
      ...
      {!flipped && (
        <p className="relative z-10 px-3 pb-2 type-caption text-text-secondary">{t('leitnerOcclusionHint')}</p>
      )}
    </div>
  );
}
