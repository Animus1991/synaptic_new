...
export function CrossLibrarySynthesisPanel({ courses, settings, lang, className }: Props) {
  ...
  const title = lang === 'el' ? 'Συνδυασμένη μελέτη' : 'Combined study';
  const subtitle =
    lang === 'el'
  ...
  const promo =
    lang === 'el'
      ? 'Συνδύασε μαθήματα σε μία απάντηση με παραπομπές.'
      : 'Combine courses into one cited answer.';
    ...
    >
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-[10px] text-violet-300/80 truncate">{promo}</span>
        </span>
