...
export const config = {
  ...
  // Replit AI integration is the default upstream; explicit env vars win.
  upstreamBaseUrl: (
    process.env.UPSTREAM_BASE_URL
    ?? process.env.AI_INTEGRATIONS_OPENAI_BASE_URL
    ?? 'https://api.openai.com/v1'
  ).replace(/\/$/, ''),
  upstreamApiKey: process.env.OPENAI_API_KEY ?? process.env.AI_INTEGRATIONS_OPENAI_API_KEY ?? '',
