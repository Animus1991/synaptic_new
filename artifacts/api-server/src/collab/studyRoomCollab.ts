...
export function startStudyRoomCollab(port: number, databaseUrl?: string): Server {
  ...
  // EADDRINUSE (e.g. tsx-watch reload race) must degrade gracefully instead of
  // crashing the whole API server via an unhandled 'error' event.
  server.httpServer.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      console.warn(`[synapse-collab] port ${port} already in use — collab websocket disabled for this run`);
    } else {
      console.warn('[synapse-collab] websocket server error:', err.message);
    }
  });
