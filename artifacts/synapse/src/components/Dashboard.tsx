...
import { Check, FileText, Loader2, RefreshCw, X } from '@/lib/lucide-shim';
...
import { Card, PrimaryCTA } from './ui/primitives';
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-brand-300 mb-1">
              {lang === 'el' ? 'AI Ημερήσια Σύνοψη' : 'AI Daily Brief'}
            </p>
            {dailyBriefLoading ? (
              <div className="flex items-center gap-2 text-xs text-text-muted">
                <Loader2 className="w-3 h-3 animate-spin" />
                {lang === 'el' ? 'Αναλύω την πρόοδό σου…' : 'Analysing your progress…'}
              </div>
            ) : (
          ...
          <div className="flex items-center gap-1 shrink-0">
            {!dailyBriefLoading && (
              <button
                type="button"
                title={lang === 'el' ? 'Ανανέωση' : 'Refresh'}
            ...
            <button
              type="button"
              title={lang === 'el' ? 'Κλείσιμο' : 'Dismiss'}
