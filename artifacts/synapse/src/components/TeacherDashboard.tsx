...
export function TeacherDashboard({
  ...
  return (
    <div data-testid="teacher-dashboard">
      <Page className="max-w-5xl mx-auto" gap="md">
        <PageHeader
          icon={Users}
          title={ui.title}
          subtitle={
            <>
              {ui.subtitle}
              {data?.syncedAt && (
                <span className="block ws-caption text-text-muted mt-1">
        ...
        {data && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatTile icon={<BookOpen className="w-5 h-5 text-brand-400" />} label={ui.courses} value={String(data.library.courseCount)} hint={ui.coursesInLibrary} />
              <StatTile icon={<FileText className="w-5 h-5 text-brand-400" />} label={ui.sourceFiles} value={String(data.library.fileCount)} hint={ui.filesUploaded} />
              <StatTile icon={<Layers className="w-5 h-5 text-brand-400" />} label={ui.topics} value={String(data.library.topicCount)} hint={ui.topicsTotal} />
              <StatTile icon={<BookMarked className="w-5 h-5 text-brand-400" />} label={ui.glossary} value={String(data.library.glossaryCount)} hint={ui.glossaryEntries} />
            </div>
            ...
            <Card className="space-y-4">
              <SectionHeading
                ...
                title={<>{ui.llmUsageMonth} · {data.usage.month}</>}
                action={
            ...
            <Card>
              <SectionHeading icon={BookOpen} iconClassName="text-brand-400" title={ui.courseRoster} />
              <p className="text-xs text-text-muted mt-1 mb-4">{ui.courseRosterHint}</p>
            ...
            <Card>
              <SectionHeading icon={PenLine} iconClassName="text-accent-cyan" title={ui.publishing} />
              <p className="text-xs text-text-muted mt-1 mb-3">{ui.publishingHint}</p>
              ...
              {data.publishing.recent.length === 0 ? (
                ...
                <ul className="space-y-2 text-xs">
                  <li className="ws-eyebrow text-text-muted">{ui.recentAnnotations}</li>
            ...
            <Card>
              <SectionHeading icon={Cpu} iconClassName="text-accent-cyan" title={ui.serverCapabilities} className="mb-3" />
        ...
        <Card>
          <SectionHeading icon={BarChart3} iconClassName="text-brand-300" title={ui.localSession} />
          <p className="text-xs text-text-muted mt-1 mb-4">{ui.localSessionHint}</p>
          {learnerModel && (
            <div className="flex flex-wrap gap-3 text-xs mb-4">
        ...
        {!settings.authToken && (
          <Card tone="muted" padding="sm" className="flex gap-3 text-sm text-text-secondary">
            <Shield className="w-5 h-5 text-brand-400 shrink-0" />
            <p>{ui.signInHint}</p>
          </Card>
        )}

        <div className="ws-caption text-text-muted flex items-center gap-1.5">
