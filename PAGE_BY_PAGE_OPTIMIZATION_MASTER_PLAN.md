...
|---|---|---|---|---|
| `landing` | `Landing.tsx` | pre-auth entry | Partial· ισχυρή παρουσίαση, αλλά μη επαληθευμένο social proof και ελλιπές legal/footer contract | Audit pending |
| `onboarding` | `Onboarding.tsx` | first-run flow | Partial· πραγματική persistence για role/time/exam, αλλά λείπει ενσωματωμένο upload step και validation contract | Audit pending |
...
`Audit complete · Truthful processing and transaction safety are P0`
...
| Demo isolation | Explicit labeling | ⏳ Pending — §7.3 |
| Legal/footer links | Real targets only | ⏳ Pending — §7.4 |

**Landing status:** `B1 partial · integrity social proof done · demo/legal pending`

### 40.5 Per-surface status sync (§1.1 ενημέρωση)

| Surface | Προηγούμενο ledger | v1.1 reconciled |
|---------|-------------------|-----------------|
| `landing` | Audit pending | B1 partial + R6 aesthetic shipped |
...
| Όλες οι υπόλοιπες §8–19, §20–24 | Audit complete · Implementation pending | **Αμετάβλητο — pending** |

### 40.6 Strict sequence — επόμενες επιτρεπτές ενέργειες

1. **Commit** uncommitted Pass 1 + master plan v1.1 + Gate A + B1 partial (on user request).
2. **B1 finish:** demo isolation banner, footer/legal, a11y evidence (§7).

### 40.7 Verification record (session 2026-07-11)
...
**Επόμενη επιτρεπτή ενέργεια:** **B1 completion** (demo/legal/a11y), μετά **Onboarding packet** κατά §29.3.
