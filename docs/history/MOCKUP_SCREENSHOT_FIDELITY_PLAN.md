# Mockup screenshot fidelity plan (Replit canvas β†’ production)

**Date:** 2026-07-16  
**Inputs:** 10 Replit canvas screenshots (DashboardΓ—2, TasksΓ—2, LibraryΓ—2, AnalyticsΓ—4)  
**Target branch:** `feat/mockup-implementation` β†’ `Animus1991/synaptic_new` only  
**Non-negotiables:** keep 100% tool functionality Β· denser type than Replit Β· **no emojis** Β· Phosphor/line icons Β· warm light + dark first-class Β· no secrets/PII in commits
...
**Density contract (user):** H1 β‰ 18β€“20px Β· section labels 11β€“13px uppercase Β· cards `p-3`/`p-4` Β· KPI numerals `text-xl` not `text-3xl` Β· zero emoji chrome.
...
| 2Γ—2 alert grid (exam/quiz/forget/misconception) | dashboard alerts + smart CTAs | **shipped** |
...
| Warm cream density | light β†’ warm-sand scope | **Wave G** |
| Compact greeting type | `text-lg` hub title | **Wave G** |

### 3.2 Tasks (shots 3β€“4)
...
| Badge copy Ξ Ξ΅ΞΞ¤Ξ•Ξ™ΞΞ•Ξ¤Ξ‘Ξ™ | `sessionRecommendedBadge` | **Wave G** (el) |
| Warm-sand page scope | already on Tasks | **shipped** |

### 3.3 Library (shots 5β€“6)
...
## 5. Wave G β€” this sprint (implementation checklist)
...
| G-03 | Light theme cream β†” mockup; density tokens tightened | CSS |
...
| ID | Item | Status |
| -- | ---- | ------ |
| H-01 | Optional Dashboard 3-column β€canvas previewβ€ layout toggle | **shipped** |
| H-02 | Library β€combined studyβ€ purple promo strip redesign | **shipped** |
...
## 6b. Wave I (screenshot IA fidelity β€” 2026-07-16)
...
- Wave K: [`MOCKUP_WAVE_K_FIDELITY_PLAN.md`](./MOCKUP_WAVE_K_FIDELITY_PLAN.md) β€” spectrum/blueprint parity; warm-sand no longer overrides spectrum; horizontal calibration bins.

## 6d. Wave L

- [`MOCKUP_WAVE_L_FIDELITY_PLAN.md`](./MOCKUP_WAVE_L_FIDELITY_PLAN.md) β€” L-1: Analytics flow banner, Library banner order, Tasks tab filter, Settings theme icons, denser Dashboard/CourseView. L-2: retrieval bar, FSRS expand polish, Visual Lab sticky footer, Agent embedded density.

## 6e. Wave M

- [`MOCKUP_WAVE_M_FIDELITY_PLAN.md`](./MOCKUP_WAVE_M_FIDELITY_PLAN.md) β€” M-1: density closeout on Onboarding/Student/Teacher/ExamPrep/Grounded/Reprocess, unicode `β“`/`β—` β†’ Phosphor Check/X, theme SoT aliases (`--color-surface-canvas`, `--color-hairline`) across warm-light/warm-sand/dark/spectrum/blueprint, `.ux-kpi-value` shared numeral utility. M-2: KPI migration (Dashboard / Analytics / CourseView / ExamPrep), Visual Lab `AnimatePresence` collapse polish, spectrum WCAG title audit, embedded Agent inline source picker.

## 6f. Wave N

- [`MOCKUP_WAVE_N_FIDELITY_PLAN.md`](./MOCKUP_WAVE_N_FIDELITY_PLAN.md) β€” N-1: state-of-the-art theme system audit β€” `--color-focus-ring` + `--color-surface-pressed` + `--color-on-brand / -danger / -warn / -success / -info` + `--elev-popover` explicit for all 5 themes, `.ux-focus-ring` shared utility, sepia `::selection` for warm-light/warm-sand, thin themed scrollbars for warm-light/warm-sand/spectrum, unicode `β•` on Agent source settings replaced with Phosphor `X`. N-2: `.ux-chip-solid-*` bound to `--color-on-*`, `.ux-elev-popover` utility applied to NotificationsPanel/CommandPalette (root + workspace)/DashboardActionHub, `--color-surface-hover-strong` + `.ux-hover-strong` (two-step hover applied to Agent embedded source menu), `--motion-duration-emphasized` (360 ms) + `--motion-ease-emphasized` (Material 3 emphasized decelerate) tokens applied to Analytics Visual Lab collapse and Agent embedded mode dropdown.

## 6g. Wave O

- [`MOCKUP_WAVE_O_FIDELITY_PLAN.md`](./MOCKUP_WAVE_O_FIDELITY_PLAN.md) β€” O-1: global `MotionConfig reducedMotion="user"` at `App.tsx` for automatic OS-level reduced-motion respect across all framer-motion subtrees; explicit emphasized transitions on Onboarding steps (welcome/role/goals/schedule), UploadModal backdrop + panel, AppToastBanner, OfflineShellBanner back-online toast. Preserves spring transitions on ConfirmDialog + DashboardHubShell overlay.

## 6h. Wave P

- [`MOCKUP_WAVE_P_FIDELITY_PLAN.md`](./MOCKUP_WAVE_P_FIDELITY_PLAN.md) β€” **P-1**: Settings 2-col masonry (`lg:columns-2 + break-inside-avoid`) kills all vertical whitespace under asymmetric-height section cards. New viz tokens per theme β€” `--viz-bar-track` / `--viz-bar-track-strong` / `--viz-bar-fill` / `--viz-bar-fill-muted` β€” retire hardcoded `#818cf8` + generic `var(--viz-track)` in Weekly Mastery bars and Dashboard mastery trend. FSRS forecast row now wraps in soft track + uses `--viz-bar-fill`. New 5-step `--color-heatmap-scale-{0..4}` tokens replace `bg-surface-hover` + `bg-brand-*` combo that collapsed to ~1.1:1 contrast on spectrum + warm-light. Weak Areas + Upcoming Exam progress tracks use `--viz-bar-track`. "Start exam simulation" CTA now uses `.ux-chip-solid-danger` with `--color-on-danger` ink (WCAG AA in every theme). Blueprint border-subtle lifted +3% white; text-muted/text-tertiary promoted one step for AA on gradient canvas.
- **P-2** (2026-07-16): Systematic global migration of every remaining `bg-surface-hover rounded-full` progress-bar track (18 call-sites across Analytics, Dashboard, Library, CourseView, TeacherDashboard, StudentOrgView, Syllabus Coverage widget, ConceptMasteryBars, CalibrationCompareBar, SignalBars, SessionQueueBar) to inline `var(--viz-bar-track)` β€” brings every functional non-text track above 3:1 vs card surface on every theme. `--viz-bar-fill-muted` per-theme mix ratios bumped from 45β€“55% β†’ 68β€“75% brand so historical Weekly Mastery bars remain visible on warm-light / warm-sand / spectrum. Analytics 3-col Courses / Concepts / Calibration row repacked with CSS multi-column masonry (`lg:columns-3 + break-inside-avoid`), eliminating 300β€“500 px whitespace under shorter Courses/Concepts columns. Recent Calibration overlay-bar (predicted+actual) migrated from `bg-accent-emerald/60` alpha to solid emerald with `opacity: 0.85` inline so both segments layer visibly. Pending-topic bullet in SyllabusCoverageWidget now filled with `var(--viz-bar-track)` + `border-border-default` for perceptibility.
...
- Companion `docs/MOCKUP_WAVE_P_FIDELITY_PLAN.md` includes exhaustive Sprint P-1/P-2/P-3 shipped logs + page Γ— theme WCAG matrix for downstream LLMs.
...
- [x] Wave H+ H-01β€¦H-05 closed without omission
