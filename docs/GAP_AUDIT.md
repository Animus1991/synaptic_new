...
| AI-06 | Tool AI intelligence layer (Phase A–D) | P1 | **in progress** | A/B: router, quiz diagnosis, PathFocus, smart cards, scratchpad/compare/debate. C: Library Ask-on-source + pin, Analytics Ask Agent, Teacher weak-topic announcement draft. D: Settings model/base-URL presets, hybrid economics + vision cost notes. Replit-redesign: unrelated history — selective ports only (Unicode debate counters + Reader ask-ai-inline); see `docs/REPLIT_REDESIGN_MERGE_AUDIT.md` |
...
| OPT-K9 | Anti-stretch content measure | P2 | **shipped** | `--content-measure` on UtilityRows, coverage, mastery bars; page stays full-width |
