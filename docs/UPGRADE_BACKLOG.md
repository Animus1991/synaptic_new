...
| D2 Mobile distribution lanes | **done** | Surfaced the production `release` lanes (iOS `upload_to_app_store`, Android Play production) as `npm run mobile:{ios,android}:release` + added `mobile:android:sync` for parity; full sync→build→beta→release ladder documented in `mobile/DISTRIBUTION.md` (per-platform tables + promotion checklist). Drift guard `src/mobileDistributionLanes.test.ts` asserts every `mobile:*` script maps to an existing Fastfile lane on the right platform. Builds on OPS-04 Fastlane/Capacitor signing already shipped |
...
| H5 | Tasks densify | **done** — Dashboard-parity hub + Canon surface kit (radius 6/10/14/20, alpha hairline, pill CTAs); no function loss (`waveTasksContract.test.ts`) |
